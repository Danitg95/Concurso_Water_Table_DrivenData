...
# Changes: Iterate over 02_categ_.R looking for hyperparameters.
#------------------
...
tic()
#-- Grid Search
mynumtre <- c(400, 500, 600)
mymtry <- c(4, 5, 6, 7)
mymnsi <- c(1, 2, 3)
migrid <- expand.grid(numtre = mynumtre, mitry = mymtry, mymnsi = mymnsi)
migrid$mierr <- 0
...
var_impor %>%
  ...
  theme_bw()
ggsave(
  paste("./charts/04_Importancia_algoritmo_ranger_variables_acierto_", 
        round(acierto_val,4),
...
fwrite(
  mysub, 
  paste("./submissions/04_ranger_acierto_", 
        round(acierto_val,4),
        "_numvars_",  nrow(var_impor), 
        "_.csv", sep =  "")
)
